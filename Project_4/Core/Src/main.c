...
void BareMetal_UART_SendChar(char c) {
	while (!(USART6 -> SR & (1 << 7)));
	USART6 -> DR = c;
}
...
RCC -> AHB1ENR |= (1 << 2); // GPIOC Clock Signal, connected to AHB1 Bus, bit 2
RCC -> APB2ENR |= (1 << 5); // USART6 Clock Signal, connected to APB2 Bus, bit 5
...
USART6 -> CR1 |= (1 << 13) | (1 << 3) | (1 << 2);
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  BareMetal_UART_SendChar('A');
	  BareMetal_UART_SendChar('\r');
	  BareMetal_UART_SendChar('\n');
	  HAL_Delay(500);
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : SPI2_CS_Pin LED1_Pin LED2_Pin LED3_Pin */
  GPIO_InitStruct.Pin = SPI2_CS_Pin|LED1_Pin|LED2_Pin|LED3_Pin;
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart){
	...
	if (huart -> Instance == USART6){
		if (incoming_data == '1'){
			...
			printf("Interrupt:Led is On\r\n");
		}
		else if (incoming_data == '0'){
			HAL_GPIO_WritePin(GPIOE, GPIO_PIN_13, GPIO_PIN_SET);
			printf("Interrupt:Led is Off\r\n");
		}
		HAL_UART_Receive_IT(&huart6, &incoming_data, 1);
	}
}
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin){
	...
	if ((interrupt_time - last_interrupt_time) > 200){
		...
		if(GPIO_Pin == GPIO_PIN_10)
			printf("Button on PE10 was Pressed!\r\n");
