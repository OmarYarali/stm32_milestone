...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	if (HAL_UART_Receive(&huart6, &incoming_data, 1, 10) == HAL_OK){
		if (incoming_data == '\r' || incoming_data == '\n'){
			incoming_buffer[incoming_index] = '\0';

			if (strcmp(incoming_buffer, "on") == 0){
				HAL_GPIO_WritePin(GPIOE, GPIO_PIN_13, GPIO_PIN_RESET);
				printf("Led is Turned ON, sir\r\n");
			}

			else if (strcmp(incoming_buffer, "off") == 0){
				HAL_GPIO_WritePin(GPIOE, GPIO_PIN_13, GPIO_PIN_SET);
				printf("Led is Turned OFF, sir\r\n");
			}
			else
		...
		incoming_index = 0;
			...
			incoming_buffer[incoming_index] = incoming_data;
			incoming_index++;
			if (incoming_index > 20)
				incoming_index = 0;
		}
	}
